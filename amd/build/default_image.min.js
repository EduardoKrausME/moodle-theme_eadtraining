define(["jquery","theme_training/trianglify"],function($,trianglify){return{generateimage:function(imageid,courseid,animate){courseid=Math.pow(courseid+1e4,2);const $img=$(`img[src*="${imageid}"],[style*="${imageid}"]`);if(!$img.length){return}const newImageHeight=$img.height();const newImageWidth=$img.width();const pattern=trianglify({height:newImageHeight,width:newImageWidth,xColors:["rgba(255, 255, 255, .6)"],yColors:"match",cellSize:25,colorFunction:trianglify.colorFunctions.shadows()});let svg=pattern.toSVG();function createRandom(seed){let x=Math.sin(seed);return Math.abs(x-Math.floor(x))}function randomColor(sum){const h=.1+createRandom(courseid+sum+1);const s=.8+createRandom(courseid+sum+2)*.2;const v=.8+createRandom(courseid+sum+3)*.2;const hsvParaRgb=(h,s,v)=>{let r,g,b;let i=Math.floor(h*6);let f=h*6-i;let p=v*(1-s);let q=v*(1-f*s);let t=v*(1-(1-f)*s);switch(i%6){case 0:r=v,g=t,b=p;break;case 1:r=q,g=v,b=p;break;case 2:r=p,g=v,b=t;break;case 3:r=p,g=q,b=v;break;case 4:r=t,g=p,b=v;break;case 5:r=v,g=p,b=q;break}return{r:Math.round(r*255),g:Math.round(g*255),b:Math.round(b*255)}};const{r,g,b}=hsvParaRgb(h,s,v);return`rgb(${r}, ${g}, ${b})`}function randon(sum,min,max){return Math.floor(createRandom(courseid+sum)*(max-min+1))+min}let extraSvg="";if(animate){extraSvg=`
                    <style>
                        #color-stop-1 {
                ...
                <path fill="url(#url-${courseid})" d="M0 0h${newImageWidth}v${newImageHeight}H0z"></path>\n`;let svgString=(new XMLSerializer).serializeToString(svg);svgString=svgString.replace(/<svg[^>]*?>/,match=>`${match}\n${extraSvg}`);const base64=btoa(unescape(encodeURIComponent(svgString)));if($img.is("img")){$img.attr("src",`data:image/svg+xml;base64,${base64}`)}else{$img.css({"background-image":`url("data:image/svg+xml;base64,${base64}")`})}}}});