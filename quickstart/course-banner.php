...
if (optional_param("POST", false, PARAM_INT)) {
    ...
    foreach ($configkeys as $name => $type) {
        ...
        if ($value !== false) {
            set_config("{$name}_{$course->id}", $value, "theme_eadtraining");
        }
    }
    ...
    redirect(new moodle_url("/course/view.php?id={$course->id}"), get_string("quickstart_banner-saved", "theme_eadtraining"));
}
...
// Course.
$bannerfileurl = theme_eadtraining_setting_file_url("banner_course_file_{$course->id}");
if (!$bannerfileurl) {
...
$action = "{$CFG->wwwroot}/theme/eadtraining/quickstart/course-banner.php?courseid={$course->id}";
...
$coursesummarycourse = get_config("theme_eadtraining", "course_summary_banner_{$course->id}");
...
$brandcolor = theme_eadtraining_default("brandcolor", "#1a2a6c", '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/', "theme_boost");
$coursesmustache = [
    ...
    "course_sections_icons" => get_config("theme_eadtraining", "course_sections_icons_{$course->id}"),

    "override_course_primarycolor" => get_config("theme_eadtraining", "override_course_primarycolor_{$course->id}"),
    "override_course_secondarycolor" => get_config("theme_eadtraining", "override_course_secondarycolor_{$course->id}"),
    "colorselect" => $OUTPUT->render_from_template("theme_eadtraining/settings/colors", [
        "coursecolor" => true,
        "colors" => $themecolors,
        "defaultcolor" => theme_eadtraining_default("override_course_primarycolor_{$course->id}", $brandcolor, '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/'),
        "defaultcolorfooter" => theme_eadtraining_default("footer_background_color", "#1a2a6c", '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/'),
        "brandcolor_background_menu" => (int) theme_eadtraining_default("brandcolor_background_menu", 0, '/^-?\d+$/'),
        "navbar_layout_is_institutional" => $OUTPUT->navbar_layout_is_institutional(),
        "secondary_color" => theme_eadtraining_secondary_color($course->id),
    ]),
];
echo $OUTPUT->render_from_template("theme_eadtraining/quickstart/courses", $coursesmustache);
$PAGE->requires->js_call_amd("theme_eadtraining/settings", "minicolors", ["override_course_primarycolor"]);
$PAGE->requires->js_call_amd("theme_eadtraining/settings", "minicolors", ["override_course_secondarycolor"]);
