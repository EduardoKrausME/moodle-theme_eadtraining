...
if (optional_param("POST", false, PARAM_INT)) {
    ...
    foreach ($homemodebanners as $template) {
        ...
        if (!$located) {
            try {
                theme_eadtraining_editor_create_page($template, $USER->lang, "home");
...
$homemustache = [
    ...
    "templates" => $templates,
    "next" => "courses",
    "all_templates" => theme_eadtraining_list_templates_category(),
];
...
$brandcolormustache = [
    ...
    "htmlselect" => $OUTPUT->render_from_template("theme_eadtraining/settings/colors", [
        "brandcolor" => true,
        "colors" => $themecolors,
        "defaultcolor" => theme_eadtraining_default("brandcolor", "#314755", '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/', "theme_boost"),
...
if (file_exists("{$CFG->libdir}/editor/tiny/lib.php")) {
    $footermustache = [
        ...
        "htmlselect" => $OUTPUT->render_from_template("theme_eadtraining/settings/colors", [
            ...
            "defaultcolor" => theme_eadtraining_default("brandcolor", "#314755", '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/'),
            "navbar_layout_is_institutional" => $OUTPUT->navbar_layout_is_institutional(),
            "secondary_color" => theme_eadtraining_secondary_color(),
        ]),
        "blocks" => [
