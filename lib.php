...
use theme_eadtraining\autoprefixer;
use theme_eadtraining\icon_extractor;
use theme_eadtraining\output\footer_renderer;
use theme_eadtraining\thumb_generator;
...
function theme_eadtraining_get_extra_scss($theme) {
    ...
    if (isset($theme->settings->scsspos[5])) {
        ...
        if ($result === true) {
            ...
            $scsspos = "
                #page::before {
                    content: 'theme_eadtraining::scsspos Error: {$result}';
                    color: #c00;
                    display: block;
                    padding: 8px 12px;
...
function theme_eadtraining_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        ...
        if ($filearea == "theme_eadtraining_customimage" || $filearea == "theme_eadtraining_customicon") {
            $thumb = (new thumb_generator())
                ->set_height(($filearea == 'theme_eadtraining_customicon') ? 50 : 150)
...
function theme_eadtraining_get_pre_scss($theme) {
    ...
    $footerbg = theme_eadtraining_default("footer_background_color", $brandcolor, '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/');
    if ($footerbg) {
    ...
    if ($CFG->theme == "degrade") {
        $angle = theme_eadtraining_default("angle", 30, '/^-?\d+$/');
        $gradient1 = theme_eadtraining_default("brandcolor_gradient_1", "#f54266", '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/');
        $gradient2 = theme_eadtraining_default("brandcolor_gradient_2", "#3858f9", '/^#[a-fA-F0-9]{6}([a-fA-F0-9]{2})?$/');
        $scss[] = "
        ...
        $topscrollbackgroundcolor = get_config("theme_eadtraining", "top_scroll_background_color");
    ...
    if ($courseid) {
        $primarycolor = get_config("theme_eadtraining", "override_course_primarycolor_{$courseid}");
        if (isset($primarycolor[3]) && preg_match('/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/', $primarycolor)) {
            $scss[] = "\$primary : {$primarycolor};";
        }

        $secondarycolor = get_config("theme_eadtraining", "override_course_secondarycolor_{$courseid}");
    ...
    if (isset($theme->settings->scsspre[5])) {
        ...
        if ($result === true) {
            ...
            $scss[] = "
                #page::before {
                    content: 'theme_eadtraining::scsspre Error: {$result}';
                    color: #c00;
                    display: block;
                    padding: 8px 12px;
                    white-space: pre-wrap;
...
function theme_eadtraining_progress_content() {
    ...
    if (!$completion->is_tracked_user($USER->id)) {
        $SESSION->notifications[] = (object) [
            "message" => get_string("notenrolledincourse", "theme_eadtraining"),
...
function theme_eadtraining_setting_file_url($setting) {
    ...
    $url = moodle_url::make_pluginfile_url($syscontext->id, "theme_eadtraining", $setting, 0, "/", $filepath);
...
function theme_eadtraining_coursemodule_standard_elements(&$formwrapper, $mform) {
    ...
    if ($CFG->theme == "eadtraining" || $CFG->theme == "eadflix") {
        ...
        $hasicons = (int) get_config("theme_eadtraining", "course_sections_icons_{$COURSE->id}");
        if ($hasicons) {
            // Background.
            if (isset($formwrapper->get_current()->coursemodule) && $formwrapper->get_current()->coursemodule) {
                ...
                $formwrapper->set_data(["theme_eadtraining_customimage" => $draftitemid]);
            }
            $mform->addElement(
                "filemanager",
                "theme_eadtraining_customimage",
        ...
        if (isset($formwrapper->get_current()->coursemodule) && $formwrapper->get_current()->coursemodule) {
            ...
            $formwrapper->set_data(["theme_eadtraining_customicon" => $draftitemid]);
        }
...
function theme_eadtraining_coursemodule_edit_post_actions($data, $course) {
    ...
    $imagearea = "theme_eadtraining_customimage";
    $iconarea = "theme_eadtraining_customicon";
    ...
    $imagefieldsubmitted = property_exists($data, "theme_eadtraining_customimage");
    $iconfieldsubmitted = property_exists($data, "theme_eadtraining_customicon");

    $hadcustomimage = theme_eadtraining_filearea_has_real_files(
        $context->id,
        $component,
        $imagearea,
        $cmid
    ...
    $hadcustomicon = theme_eadtraining_filearea_has_real_files(
        $context->id,
        $component,
        $iconarea,
        $cmid
    ...
    if ($imagefieldsubmitted) {
        file_save_draft_area_files(
            (int) $data->theme_eadtraining_customimage,
            $context->id,
            $component,
            $imagearea,
            $cmid,
        ...
        $hascustomimage = theme_eadtraining_filearea_has_real_files(
            $context->id,
            $component,
            $imagearea,
            $cmid
        ...
        $name = "theme_eadtraining_customimage_{$cmid}";
        if ($hascustomimage) {
            set_config($name, 1, "theme_eadtraining");
        } else {
            unset_config($name, "theme_eadtraining");
        }
    ...
    if ($iconfieldsubmitted) {
        file_save_draft_area_files(
            (int) $data->theme_eadtraining_customicon,
            $context->id,
            $component,
            $iconarea,
            $cmid,
        ...
        $hascustomicon = theme_eadtraining_filearea_has_real_files(
            $context->id,
            $component,
            $iconarea,
            $cmid
        ...
        $name = "theme_eadtraining_customicon_{$cmid}";
        if ($hascustomicon) {
            set_config($name, 1, "theme_eadtraining");
        } else {
            unset_config($name, "theme_eadtraining");
    ...
    if (property_exists($data, "theme_eadtraining_customcolor")) {
        $name = "theme_eadtraining_customcolor_{$cmid}";
        $color = trim((string) $data->theme_eadtraining_customcolor);

        if ($color === "") {
            unset_config($name, "theme_eadtraining");
        } else {
            set_config($name, $color, "theme_eadtraining");
        }
        ...
        if (!empty($areafiles)) {
            ...
            if (in_array($mimetype, $supported, true)) {
                try {
                    ...
                    $tmpdir = make_temp_directory("theme_eadtraining_icons");
 ...
 * @throws \coding_exception
 */
function theme_eadtraining_filearea_has_real_files(int $contextid, string $component, string $filearea, int $itemid): bool {
    $fs = get_file_storage();
 ...
 * @throws dml_exception
 */
function theme_eadtraining_default($configname, $default, $regexvalidate, $plugin = "theme_eadtraining") {
