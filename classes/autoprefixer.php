...
class autoprefixer {
    ...
    public function __construct(Document $tree) {
        debugging('theme_eadtraining\autoprefixer() is deprecated. Required prefixes for Bootstrap ' .
     ...
     */
    protected function manipulate_rule_values(array $rules) {
        $finalrules = [];
     ...
     */
    protected function process_block($block) {
        foreach ($block->getContents() as $node) {
            ...
            if ($node instanceof CSSList) {
                $this->process_block($node);

            } else if ($node instanceof RuleSet) {
                $this->process_declaration($node, $block);
            }
        }
    }
     ...
     */
    protected function process_declaration($node, $parent) {
        $rules = [];
        ...
        $node->setRules($this->manipulate_rule_values($rules));
