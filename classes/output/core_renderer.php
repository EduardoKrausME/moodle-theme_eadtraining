    ...
    public function standard_head_html() {
        ...
        foreach ($urls as $url) {
            ...
            if (!empty($this->page->course) && !empty($this->page->course->id) && $this->page->course->id > 1) {
                $coursecolor = (string)get_config("theme_eadtraining", "override_course_color_{$this->page->course->id}");
            }

            // Check profile.
            $profileid = 0;
            $plugins = get_plugins_with_function("theme_eadtraining_get_active_profile_id");
            foreach ($plugins as $plugintype => $names) {
                foreach ($names as $pluginname => $functionname) {
            ...
            if (isset($coursecolor[3]) || $profileid > 0) {
                ...
                $newurl = preg_replace_callback(
                    ...
                    function ($matches) {
                        ...
                        return "eadtraining/{$novoid}_{$this->page->course->id}/";
                    },
                    $newurl
                );
